...
export async function processMoroccanCinBuffers({ rectoBuf, versoBuf }) {
  ...
  const rectoRequested = Boolean(rectoBuf?.length);
  const versoRequested = Boolean(versoBuf?.length);
  const mindeeRectoOk = !rectoRequested || Boolean(recto?.fields && Object.keys(recto.fields).length);
  const mindeeVersoOk = !versoRequested || Boolean(verso?.fields && Object.keys(verso.fields).length);
  const mindeeAnyOk = (rectoRequested && mindeeRectoOk) || (versoRequested && mindeeVersoOk);
  const diagnostics = buildDiagnostics({
    rectoBuf,
    versoBuf,
    sideErrors,
    mindee_recto_ok: rectoRequested ? mindeeRectoOk : null,
    mindee_verso_ok: versoRequested ? mindeeVersoOk : null,
    endpoint_used: routing.endpoint,
    mindee_http_called: auditState.mindee_http_called,
  });

  function resolveFailureCode() {
    if (sideErrors.some((e) => /402|subscription period has ended/i.test(e))) return 'MINDEE_SUBSCRIPTION_EXPIRED';
    if (sideErrors.some((e) => e.includes('MINDEE_MODEL_ID'))) return 'MINDEE_MODEL_ID_MISSING';
    return 'MINDEE_EMPTY_EXTRACTION';
  }

  if (!mindeeAnyOk) {
    const detail = sideErrors.length ? sideErrors.join(' | ') : 'Mindee: aucune extraction.';
    const code = resolveFailureCode();
    console.error('[OCR CIN] provider_final=tesseract —', detail);
    const userHint = code === 'MINDEE_SUBSCRIPTION_EXPIRED'
      ? 'Abonnement Mindee expiré (402) — renouvelez sur app.mindee.com puis réessayez.'
      : detail + ' Mindee non utilisé — fallback Tesseract.';
    return {
      success: false,
      code,
      allowFallback: true,
      error: userHint,
      provider_final: 'tesseract',
      mindee_recto_ok: rectoRequested ? mindeeRectoOk : false,
      mindee_verso_ok: versoRequested ? mindeeVersoOk : false,
      mindee_http_called: auditState.mindee_http_called,
      diagnostics,
      audit: {
        mindee_http_called: auditState.mindee_http_called,
  ...
  const rectoOkOut = rectoRequested ? mindeeRectoOk : false;
  const versoOkOut = versoRequested ? mindeeVersoOk : false;
  ...
  return {
    ...
    recto,
    verso,
    mindee_recto_ok: rectoOkOut,
    mindee_verso_ok: versoOkOut,
    mindee_http_called: auditState.mindee_http_called,
    diagnostics,
    audit: {
      mindee_http_called: auditState.mindee_http_called,
