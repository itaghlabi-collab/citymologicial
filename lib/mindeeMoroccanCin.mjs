...
export function getMindeeRoutingInfo(apiKey = process.env.MINDEE_API_KEY) {
  ...
  const modelMissing = Boolean(v2 && key && !modelId);
  return {
    OCR_PROVIDER: (process.env.OCR_PROVIDER || 'mindee').toLowerCase(),
    key_type: !key ? 'missing' : (v2 ? 'md_v2' : 'legacy_v1'),
    has_MINDEE_MODEL_ID: Boolean(modelId),
    api_version: modelMissing ? 'blocked' : (v2 && modelId ? 'v2' : 'v1'),
    endpoint: modelMissing ? null : (v2 && modelId ? MINDEE_V2_ENQUEUE : MINDEE_V1),
    product: v2 && modelId ? 'custom_extraction_v2' : 'international_id/v2',
    mindee_api_key: key ? 'present' : 'missing',
    mindee_model_id: modelId ? 'present' : (v2 ? 'missing' : 'not_required_v1'),
    model_id_required: modelMissing,
  };
}
...
function logMindeeServerConfig() {
  ...
  if (info.model_id_required) {
    console.error('[OCR CIN] MINDEE_MODEL_ID manquant — aucun appel HTTP Mindee ne sera effectué (clé md_*)');
  }
}
...
function validateMindeeConfig() {
  ...
  if (isMindeeV2Key(apiKey) && !resolveMindeeModelId()) {
    return { ok: false, code: 'MINDEE_MODEL_ID_MISSING', error: MINDEE_MODEL_ID_ERROR };
  }
  return { ok: true };
}
...
async function mindeeV2Predict(buffer, mime, label, apiKey) {
  const modelId = resolveMindeeModelId();
  if (!modelId) {
    const err = new Error(MINDEE_MODEL_ID_ERROR);
