...
import {
  Trash2, RotateCcw, Download, Search, AlertTriangle, Clock, FileText
} from 'lucide-react';
...
export default function Corbeille({ deletedDocs, onRestore, onPermanentDelete, onEmptyTrash }) {
  ...
  const filtered = docs.filter(d => {
    ...
    return !q || d.nom.toLowerCase().includes(q) || (d.supprime_par || '').toLowerCase().includes(q);
  });
                  ...
                  <tr key={d.id} style={{ opacity: 0.85 }}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          <div style={{ fontWeight: 600, fontSize: '0.85rem', textDecoration: 'line-through', color: 'var(--text-3)' }}>{d.nom}</div>
