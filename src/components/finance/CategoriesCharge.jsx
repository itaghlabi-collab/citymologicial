...
import { Tag, Plus, Edit2, Trash2, Archive, ToggleLeft, Search, Filter, Download } from 'lucide-react';
import {
...
export default function CategoriesCharge({ charges }) {
  ...
  const handleSave = useCallback((data) => {
    if (editCat) {
      ...
      setCats(prev => [...prev, { ...data, id: genId(), date_creation: today, charges_liees: 0 }]);
    }
    setShowModal(false);
    setEditCat(null);
  }, [editCat, today]);
  ...
  function handleToggleStatut(id, newStatut) {
    setCats(prev => prev.map(c => c.id === id ? { ...c, statut: newStatut } : c));
  }
      ...
      <div className="card" style={{ padding: 0 }}>
        {filtered.length === 0 ? (
          <EmptyState
            ...
            sub="Créez votre première catégorie de charge"
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: 4 }}>
                        ...
                        <button
                          ...
                          onClick={() => handleToggleStatut(c.id, c.statut === 'Active' ? 'Inactive' : 'Active')}
                        >
                          <ToggleLeft size={13} />
                        </button>
                        <button
                          className="btn btn-ghost btn-sm"
                          title="Archiver"
                          onClick={() => handleToggleStatut(c.id, 'Archivée')}
                        >
                          <Archive size={13} />
                        </button>
