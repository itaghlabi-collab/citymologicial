...
let feuilleCaisseMaintenanceUiApplied = false;
let rhAuditOncePromise = null;
function auditRhFinanceSyncOnce() {
  if (!rhAuditOncePromise) rhAuditOncePromise = auditRhFinanceSync();
  return rhAuditOncePromise;
}
...
export default function FeuilleCaisse() {
  ...
  useEffect(() => {
    if (!configured || viewLoading || feuilleCaisseMaintenanceUiApplied) return undefined;
    let cancelled = false;
    (async () => {
      try {
        const r = await runRhPaymentsCashBackfillOnce();
        let recon = { txCancelled: 0, chargesCancelled: 0, syncedMissing: 0 };
        try {
          recon = await reconcileDepensesCourantesCashOnce();
        } catch (dupErr) {
          console.warn('[CITYMO] reconcile dépenses ↔ caisse', dupErr);
        }
        if (cancelled) return;
        feuilleCaisseMaintenanceUiApplied = true;
      ...
      if (!cancelled) {
        auditRhFinanceSyncOnce().then(setRhAudit).catch(() => setRhAudit(null));
      }
    })();
    return () => { cancelled = true; };
  }, [configured, viewLoading]);
