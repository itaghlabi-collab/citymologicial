/**
 * inventaire/shared.jsx — Composants partagés module Inventaire & Dépôt ERP CITYMO
 */
...
export const EMPLACEMENTS_STOCK = [
  ...
  'CHANTIER LOGIPARC',
  'CHANTIER VILLA POLO',
  'CHANTIER VILLA BENSOUDA',
  'CHANTIER VILLA BOUSKOURA',
  'CHANTIER MEDAFRICA BELVEDER',
  'CHANTIER AERONAUTICA RACINE',
  'CHANTIER LAMARTINE',
  "ATELIER D'ALUMINIUM",
  'ATELIER DE FERRONNERIE',
  'BUREAU CITYMO BD MED 5',
  'GISSAH KIOSQUES',
  'GISSAH STORE MOROCCO MALL',
  'DIVERS',
];
...
/** Badge stock alert */
export function StockAlert({ qte, seuil }) {
  if (!seuil || seuil === '' || Number(seuil) === 0) return null;
  const q = Number(qte) || 0;
  const s = Number(seuil);
  if (q === 0) return <span className="badge badge-red" style={{ fontSize: '0.68rem', marginLeft: 6 }}>Rupture</span>;
  if (q <= s * 0.5) return <span className="badge badge-red" style={{ fontSize: '0.68rem', marginLeft: 6 }}>Critique</span>;
  if (q <= s) return <span className="badge badge-orange" style={{ fontSize: '0.68rem', marginLeft: 6 }}>Bas</span>;
  return null;
}
