...
export default function PlanningWbsView({ tasks, onEdit, onAddChild }) {
  ...
  /* Mobile : cartes uniquement (pas de tableau dans le DOM) */
  if (isMobile) {
    return (
      <div className="pj-wbs pj-wbs--mobile" aria-label="Structure WBS">
        {rows.map((row) => {
          ...
          return (
            ...
            >
              ...
              {row.lot ? <div className="pj-wbs-lot">{row.lot}</div> : null}
              ...
              <footer className="pj-wbs-card-actions">
                {row.hasChildren && (
                  <button type="button" className="btn btn-secondary btn-sm" onClick={() => toggle(row.id)}>
                ...
                <button type="button" className="btn btn-ghost btn-sm pj-wbs-add-child" onClick={() => onAddChild?.(row)}>
  ...
  return (
    <div className="pj-wbs pj-wbs--desktop" style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden', background: '#fff' }}>
