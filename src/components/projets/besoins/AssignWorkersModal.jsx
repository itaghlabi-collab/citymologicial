/**
 * AssignWorkersModal.jsx — Affectation ouvriers avec filtres (projet / besoin)
 */
import { useState, useMemo, useEffect } from 'react';
import { Search, X, Loader2 } from 'lucide-react';
import { listWorkers } from '../../../services/rh/workers';
import { listWorkersByProject, saveProjectWorkerAssignments } from '../../../services/rh/workerProjectAssignments';
...
function workerDisponible(w, projectId) {
  ...
  if (onThis && others.length === 0) return { label: 'Sur ce chantier', ok: true };
  if (others.length > 0) return { label: 'Occupé', ok: false };
  return { label: 'Disponible', ok: true };
}

export default function AssignWorkersModal({
  open, onClose, projet, need = null, onSaved, saving: externalSaving,
}) {
  ...
  const [filters, setFilters] = useState({
    search: '', fonction: '', dispo: '', ville: '', qualification: '', permis: '',
  });

  useEffect(() => {
    if (!open || !projet?.id) return;
    setLoading(true);
    Promise.all([listWorkers(), listWorkersByProject(projet.id)])
      .then(([all, wa]) => {
        setWorkers(all || []);
        setAssigned(wa || []);
        setSelected(new Set((wa || []).map((a) => String(a.workerId))));
      })
  ...
  const filtered = useMemo(() => {
    ...
    const targetMetier = need?.corps_metier ? normBesoinFonction(need.corps_metier) : '';
    return (workers || []).filter((w) => {
      if (w.statut !== 'actif') return false;
      ...
      if (targetMetier && normBesoinFonction(w.fonction) !== targetMetier) return false;
      const disp = workerDisponible(w, projet.id);
      if (filters.dispo === 'dispo' && !disp.ok) return false;
      if (filters.dispo === 'occupe' && disp.ok && !(w.assigned_project_ids || []).includes(String(projet.id))) return false;
      ...
      if (filters.permis && !(w.badge || '').toLowerCase().includes(filters.permis.toLowerCase())) return false;
      return true;
    });
  }, [workers, filters, need, projet?.id]);
    ...
    <div className="rh-emp-modal-overlay" style={{ zIndex: 1400 }}>
      <div className="card" style={{ width: 'min(98vw, 960px)', maxHeight: '94vh', display: 'flex', flexDirection: 'column', padding: 0 }}>
        <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderBottom: '1px solid var(--border)' }}>
          <div>
            <div style={{ fontSize: '0.68rem', fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase' }}>Affectation ouvriers</div>
            <h3 style={{ margin: 0, fontWeight: 800, fontFamily: 'var(--font-head)' }}>
              {need ? `${need.fonction} — ${projet?.nom}` : projet?.nom}
            </h3>
          </div>
        ...
        <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: 8 }}>
          ...
          <select value={filters.dispo} onChange={(e) => setFilters((p) => ({ ...p, dispo: e.target.value }))} style={IS}>
            <option value="">Dispo / Occupé</option>
          ...
          <input value={filters.permis} onChange={(e) => setFilters((p) => ({ ...p, permis: e.target.value }))} placeholder="Permis / badge" style={IS} />
        </div>
            ...
            <table style={{ width: '100%', fontSize: '0.82rem', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '2px solid var(--border)', background: '#FAFAFA' }}>
                  ...
                  <th style={{ padding: 8 }}>Disponible</th>
              ...
              <tbody>
                {filtered.map((w) => {
                  ...
                  const otherProjects = (w.assigned_project_ids || []).filter((id) => String(id) !== String(projet.id)).length;
                  return (
                    <tr key={w.id} style={{ borderBottom: '1px solid var(--border)', background: checked ? '#FFF5F5' : 'transparent' }}>
                      ...
                      <td style={{ padding: 8 }}>
                        <span className={`badge ${disp.ok ? 'badge-green' : 'badge-orange'}`}>{disp.label}</span>
                      </td>
        ...
        <footer style={{ padding: '14px 20px', borderTop: '1px solid var(--border)', display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: '0.82rem', color: 'var(--text-3)', marginLeft: 8 }}>{selected.size} sélectionné(s)</span>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button type="button" className="btn btn-secondary" onClick={onClose} disabled={busy}>Annuler</button>
            <button type="button" className="btn btn-primary" onClick={handleSave} disabled={busy}>
              {busy ? <Loader2 size={14} className="spin" /> : null} Affecter la sélection
            </button>
          </div>
        </footer>
      </div>
