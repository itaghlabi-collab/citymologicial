/**
 * receptionChecklistPdf.js — PDF liste de réception / checklist devis (cases à cocher)
 */
...
function drawCheckbox(doc, cx, cy, size = CHECKBOX_SIZE) {
  const x = cx - size / 2;
  const y = cy - size / 2;
  doc.setDrawColor(...BORDER);
...
export async function generateReceptionChecklistPdf(devis, catMap = {}) {
  ...
  const drawTableHeader = (startY) => {
    ...
    textCenter(doc, 'CHECK', COL_X[4] + COL_W[4] / 2, mid);
  ...
  const drawEmptyRow = (startY) => {
    ...
    doc.text('Aucune ligne article', M + CONTENT_W / 2, startY + 7.5, { align: 'center' });
    return startY + 12;
  };
  ...
  const drawArticleRow = (row, startY) => {
    ...
    drawCheckbox(doc, COL_X[4] + COL_W[4] / 2, midY);
  ...
  doc.text(`CHECKLIST ${devis.reference || ''}`, M, y);
  y += 3;
  doc.setDrawColor(...RED);
  ...
  doc.setFontSize(8);
  doc.setTextColor(...MUTED);
  doc.text('Liste de réception — à valider par le chef de projet (cocher chaque ligne réceptionnée).', M, y);
  ...
  doc.save(`Checklist_${(devis.reference || 'reception').replace(/\s+/g, '_')}.pdf`);
}
