...
/** Sources manuelles (créées depuis Feuille de caisse) — éditables / suppressibles. */
const MANUAL_SOURCE_TYPES = new Set([
  FINANCE_SOURCE_TYPES.CASH_FUNDING,
]);

export function isAutoGeneratedTransaction(tx) {
  if (!tx) return false;
  if (tx.charge_id || tx.payment_order_id) return true;
  if (tx.is_auto_generated) return true;
  // Alimentation caisse : source_type cash_funding mais opération manuelle.
  if (tx.source_type && !MANUAL_SOURCE_TYPES.has(tx.source_type)) return true;
  return false;
}
