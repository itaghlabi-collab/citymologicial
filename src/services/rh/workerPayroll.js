...
import {
  clearWorkerPaymentExpenseLinks,
  syncWorkerPaymentExpenseOutcome,
} from '../finance/workerPaymentExpenseSync';
...
export function toWorkerPayrollRow(line, batchMeta) {
  ...
  if (batchMeta.importBatchId) row.import_batch_id = batchMeta.importBatchId;
  if (batchMeta.importSource) row.import_source = batchMeta.importSource;
  if (batchMeta.importRef) row.import_ref = batchMeta.importRef;
  return row;
}
...
/** Même logique que le tableau Paiement hebdo (toutes semaines) : fusion + présences. */
function mergePayrollRowsForDisplay(rows) {
  const list = (rows || []).filter((p) => p && p.statut !== 'Annulé');
  if (!list.length) return null;
  return list.reduce((acc, row, i) => {
    if (i === 0) return { ...row, weeklyRecords: [row], mergeAllWeeks: true };
    return mergePayrollLines(acc, row);
  });
}

/**
 * Net à payer caisse = champ « Net à payer » du tableau (enrichPayrollWithAttendance).
 * Ne pas utiliser montant_net DB seul — peut être obsolète si semaine déjà Payée.
 */
export function computeWorkerNetToPayForCash(activeRows, attendance = []) {
  const merged = mergePayrollRowsForDisplay(activeRows);
  if (!merged) return 0;
  const enriched = enrichPayrollWithAttendance(merged, attendance);
  return round2(Number(enriched?.total) || 0);
}
...
/** Entité caisse consolidée : montant = Net à payer affiché (présences), pas montant_net DB. */
function buildConsolidatedWorkerPaymentEntity(activeRows, attendance = []) {
  ...
  const merged = mergePayrollRowsForDisplay(activeRows);
  const enriched = enrichPayrollWithAttendance(merged || latest, attendance);
  const netToPay = round2(Number(enriched?.total) || 0);
  return {
    workerId: latest.workerId,
    projectId: latest.projectId,
    ouvrier: latest.ouvrier,
    projet: latest.projet,
    paid_amount: netToPay,
    net_to_pay: netToPay,
    net_amount: netToPay,
    total_net: netToPay,
    montantNet: netToPay,
    total: netToPay,
    montant_net: netToPay,
    paymentDate: enriched?.paymentDate || resolveConsolidatedCashPaymentDate(sorted),
...
export function canSyncWorkerPaymentGroup(rows, attendance = []) {
  ...
  const totalNet = computeWorkerNetToPayForCash(activeRows, attendance);
 ...
 * — ATELIER / DÉPÔT → dépense générale Main-d'œuvre interne (hors projet)
 */
export async function syncWorkerPaymentGroup(workerId, projectId, options = {}) {
  ...
  const attendance = options.attendance ?? await listAttendance().catch(() => []);
  ...
  if (!activeRows.length) {
    await clearWorkerPaymentExpenseLinks(sourceType, sourceId);
    return removeLinkedFinanceTransaction(sourceType, sourceId);
  }

  const gate = canSyncWorkerPaymentGroup(activeRows, attendance);
  if (!gate.ok) {
  ...
  const entity = buildConsolidatedWorkerPaymentEntity(activeRows, attendance);
  console.log('[FINANCE SYNC] net_to_pay caisse', {
    workerId, ouvrier: entity.ouvrier, net: entity.net_to_pay, date: entity.paymentDate,
  });
  const financeResult = await syncFinanceTransaction(sourceType, sourceId, { entity });
  try {
    const expenseResult = await syncWorkerPaymentExpenseOutcome({ entity, sourceType, sourceId });
    if (import.meta.env?.DEV) {
      console.log('[WORKER PAYMENT EXPENSE SYNC]', expenseResult);
    }
  } catch (err) {
    console.warn('[CITYMO] expense outcome after worker payment', err);
  }
...
/** Audit : groupes ouvrier×projet Payé vs ligne caisse consolidée (montant + date exacts). */
export async function auditWorkerPayrollCashSync(client) {
  ...
  const { data: txs, error } = await client
    ...
    .select('source_id, montant, date_operation, statut, source_type')
    .in('source_type', ['worker_payment', 'worker_weekly_payment'])
    .neq('statut', 'Annulé');
  if (error) throw error;

  const txBySource = new Map((txs || []).map((t) => [t.source_id, t]));
  const groups = new Map();
  for (const p of rows) {
    const key = payrollGroupKey(p.workerId, p.projectId);
  ...
  for (const [key, groupRows] of groups) {
    ...
    const gate = canSyncWorkerPaymentGroup(groupRows, attendance);
    if (!gate.ok) continue;

    expectedGroups += 1;
    const sourceId = await workerPaymentSourceId(workerId, projectId || '');
    const tx = txBySource.get(sourceId);
    const entity = buildConsolidatedWorkerPaymentEntity(
      groupRows.filter((p) => p.statut !== 'Annulé'),
      attendance,
    );
    const expectedNet = round2(entity.net_to_pay);
    const expectedDate = String(entity.paymentDate || '').slice(0, 10);

    const montantOk = tx && Math.abs(Number(tx.montant) - expectedNet) < 0.01;
    const dateOk = tx && String(tx.date_operation || '').slice(0, 10) === expectedDate;

    if (!tx || !montantOk || !dateOk) missingOrWrong += 1;
  }

  return { expectedGroups, missingOrWrong, workerTxCount: (txs || []).length };
}
...
export async function backfillWorkerPayrollToCash() {
  ...
  for (const { workerId, projectId } of groups.values()) {
    try {
      const r = await syncWorkerPaymentGroup(workerId, projectId, { allRecords: rows, attendance });
...
/**
 * Masque les lignes sans présence valide nouvelle logique (y compris Payé legacy en UI).
 * Les données restent en base pour la feuille de caisse / historique.
 */
export function filterPayrollForNewLogicOnly(records, attendance = []) {
  const active = filterActiveAttendance(attendance);

  const activeWorkerProject = new Set();
  const activeWeekKeys = new Set();
  active.forEach((att) => {
    if (!att.workerId || !att.date) return;
    const pid = att.projectId || att.workerProjectId;
    if (!pid) return;
    activeWorkerProject.add(`${att.workerId}|${pid}`);
    activeWeekKeys.add(`${att.workerId}|${pid}|${weekStartMonday(att.date)}`);
  });

  let excluded = 0;
  const filtered = (records || []).filter((p) => {
    const wpKey = `${p.workerId}|${p.projectId}`;

    if (!activeWorkerProject.has(wpKey)) {
      excluded += 1;
      return false;
    }

    if (!p.autoGenerated) return true;

    const weekKey = `${p.workerId}|${p.projectId}|${p.semaineDebut}`;
    if (activeWeekKeys.has(weekKey)) return true;
    excluded += 1;
    return false;
  });

  if (excluded > 0) {
    console.log('[PAYROLL FILTER] only project_assignment presences used', { excluded, kept: filtered.length });
  }
  return filtered;
}
...
function payrollGroupKey(workerId, projectId) {
...
export async function syncWorkerPayrollFromAttendance(ctx) {
  ...
  const importNote = ctx.importMeta?.note || '';
  const importBatchId = ctx.importMeta?.batchId || null;
  const importSource = ctx.importMeta?.source || null;
  const importRef = ctx.importMeta?.ref || null;
  ...
  for (const draft of drafts) {
    ...
    if (match && (match.statut === 'Payé' || match.statut === 'Annulé')) {
      skippedPaid += 1;
      continue;
    }
    ...
    const prevNotes = match?.notes || '';
    const notes = [prevNotes, importNote].filter((x) => String(x || '').trim()).join(' · ');
    ...
    if (match) {
      let { error } = await getSupabase()
        ...
        .update(row)
        .eq('id', match.id);
      if (error && /import_batch_id|import_source|import_ref|column|schema cache/i.test(error.message || '')) {
        const fallback = { ...row };
        delete fallback.import_batch_id;
        delete fallback.import_source;
        delete fallback.import_ref;
        ({ error } = await getSupabase().from(TABLE).update(fallback).eq('id', match.id));
      }
      if (error) {
      ...
      let { error } = await getSupabase().from(TABLE).insert(row);
      if (error && /import_batch_id|import_source|import_ref|column|schema cache/i.test(error.message || '')) {
        const fallback = { ...row };
        delete fallback.import_batch_id;
        delete fallback.import_source;
        delete fallback.import_ref;
        ({ error } = await getSupabase().from(TABLE).insert(fallback));
      }
      if (error) {
  ...
  return { created, updated, removed, skippedPaid };
}
