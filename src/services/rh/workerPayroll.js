 ...
 * - Lit public.payroll (Payé réels ; brouillons auto En attente ignorés)
 * - Montant = somme montant_net des lignes Payé
 * - Date = payment_date (ou aujourd'hui si absent)
...
export async function syncWorkerPaymentGroup(workerId, projectId, options = {}) {
  ...
  console.log('[FINANCE SYNC] syncWorkerPaymentGroup', {
    workerId, projectId, sourceType, sourceId, payrollRows: rows.length,
  });
  ...
  if (!activeRows.length) {
    console.log('[FINANCE SYNC SKIP] aucune ligne active', { workerId, projectId });
    return removeLinkedFinanceTransaction(sourceType, sourceId);
  }

  const gate = canSyncWorkerPaymentGroup(activeRows);
  if (!gate.ok) {
    console.warn('[FINANCE SYNC SKIP]', gate.reason, {
      workerId,
      projectId,
      statuts: activeRows.map((p) => ({
        id: p.id, statut: p.statut, semaine: p.semaineDebut, auto: p.autoGenerated,
      })),
    });
    return removeLinkedFinanceTransaction(sourceType, sourceId);
  }
