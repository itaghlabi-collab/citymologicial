...
/**
 * Masque les lignes auto-générées sans présence valide (nouvelle logique).
 * Les paiements déjà Payé / Annulé sont conservés.
 */
export function filterPayrollForNewLogicOnly(records, attendance = []) {
  ...
  active.forEach((att) => {
    ...
    if (!pid) return;
    activeKeys.add(`${att.workerId}|${pid}|${weekStartMonday(att.date)}`);
  });

  let excluded = 0;
  const filtered = (records || []).filter((p) => {
    if (p.statut === 'Payé' || p.statut === 'Annulé') return true;
    if (!p.autoGenerated) return true;
    const key = `${p.workerId}|${p.projectId}|${p.semaineDebut}`;
