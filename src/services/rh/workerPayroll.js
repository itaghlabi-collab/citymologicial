...
import { workerFullName, listAttendance, computeAttendanceWorkMetrics, filterAttendanceForWorkerWeek, sumWorkerAttendanceFromRecords } from './attendance';
...
/** Regroupe les lignes de paiement par projet + semaine. */
export function groupPayrollByProjectWeek(records, weekFilter = '') {
  const groups = new Map();

  for (const r of records || []) {
    if (weekFilter && r.semaineDebut !== weekFilter) continue;
    const key = `${r.projectId || r.projet}|${r.semaineDebut}`;
    if (!groups.has(key)) {
      groups.set(key, {
        projectId: r.projectId,
        projet: r.projet || '—',
        semaineDebut: r.semaineDebut,
        semaineFin: r.semaineFin || weekEndSunday(r.semaineDebut),
    ...
    g.lignes.push(r);
    g.totalNet = round2(g.totalNet + (Number(r.total) || 0));
    g.totalBrut = round2(g.totalBrut + (Number(r.montantBrut) || 0));
...
/** Recalcule jours, heures et montants depuis les présences de la semaine. */
export function enrichPayrollWithAttendance(payrollLine, attendance = []) {
  ...
  const attSum = sumWorkerAttendanceFromRecords(
    attendance,
