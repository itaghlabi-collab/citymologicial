...
export function formatSupabaseError(error, fallback = 'Une erreur est survenue.') {
  ...
  if (message.includes('internal_task_dg_relances')) {
    return 'Table relances DG absente — exécutez supabase/RUN_INTERNAL_TASKS_DG_RELANCES.sql dans Supabase → SQL Editor.';
  }
