-- CITYMO — Workflow Achats complet (devis, historique, ordres d'achat, liens OP)
-- Exécuter dans Supabase SQL Editor après RUN_PURCHASE_REQUESTS.sql

CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- ── Colonnes workflow sur purchase_requests ───────────────────────────────────
...
-- ── Devis fournisseurs liés à une demande ───────────────────────────────────
...
-- ── Ordres d'achat (OA) — distinct des bons de commande (purchase_orders) ─────
...
-- FK purchase_requests → quote / OA (après création tables)
DO $fk$
BEGIN
  IF NOT EXISTS (
...
-- ── Migration statuts legacy → workflow ───────────────────────────────────────
...
-- Assignation automatique Laila WOTFI si vide
...
-- ── RLS (même politique que les autres tables achats) ─────────────────────────
