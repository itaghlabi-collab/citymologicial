-- =============================================================================
-- CITYMO — Seed ELECTRICITE Consommable — Plinthes / supports prise
...
DROP TABLE IF EXISTS tmp_seed_electricite_plinthes;
CREATE TEMP TABLE tmp_seed_electricite_plinthes (
  nom text PRIMARY KEY,
  qty numeric NOT NULL
);

INSERT INTO tmp_seed_electricite_plinthes (nom, qty) VALUES
  ('SUPPORT PRISE 45 x 45 - 3 MODULES POUR PLINTHE',     16),
  ('SUPPORT PRISE 45 x 45 - 2 MODULES POUR PLINTHE',      2),
  ('SUPPORT PRISE 45 x 45 - 1 MODULE POUR PLINTHE',       2),
  ('EMBOUT POUR PLINTHE 100 x 45',                        5),
  ('ANGLE INTÉRIEUR POUR PLINTHE 100 x 45',              16),
  ('ANGLE PLAT POUR PLINTHE 100 x 45',                    7),
  ('ANGLE EXTÉRIEUR POUR PLINTHE 100 x 45',               3);
...
to_insert AS (
  ...
  FROM tmp_seed_electricite_plinthes s
  WHERE NOT EXISTS (
...
SET
  ...
  updated_at   = NOW()
FROM public.stock_categories c, tmp_seed_electricite_plinthes s
WHERE lower(trim(a.nom)) = lower(trim(s.nom))
  AND (
...
BEGIN
  IF EXISTS (
    ...
    WHERE (a.barcode_value IS NULL OR trim(a.barcode_value) = '')
      ...
      AND EXISTS (
        SELECT 1 FROM tmp_seed_electricite_plinthes s
        WHERE lower(trim(a.nom)) = lower(trim(s.nom))
      );
  END IF;
END $bc$;
...
WHERE NOT EXISTS (
  ...
  WHERE m.article_id = a.id
    AND (
      coalesce(m.payload->>'source', '') = 'seed_electricite_plinthes'
      OR (
...
FROM public.stock_articles a
JOIN tmp_seed_electricite_plinthes s ON lower(trim(a.nom)) = lower(trim(s.nom))
LEFT JOIN public.stock_levels l
  ON l.article_id = a.id
 AND lower(trim(coalesce(l.emplacement, ''))) = 'depot lakhyayta'
ORDER BY a.nom;

SELECT COUNT(*)::int AS articles_seed_electricite_plinthes
FROM public.stock_articles a
JOIN tmp_seed_electricite_plinthes s ON lower(trim(a.nom)) = lower(trim(s.nom));
