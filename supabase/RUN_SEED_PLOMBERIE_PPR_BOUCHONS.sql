-- =============================================================================
-- CITYMO — Seed PLOMBERIE Consommable — Complément PPR (bouchons + réduction)
...
DROP TABLE IF EXISTS tmp_seed_plomberie_ppr_bouchons;
CREATE TEMP TABLE tmp_seed_plomberie_ppr_bouchons (
  nom text PRIMARY KEY,
  qty numeric NOT NULL
);

INSERT INTO tmp_seed_plomberie_ppr_bouchons (nom, qty) VALUES
  ('RÉDUCTION PPR 25 x 20',     8),
  ('CHAPEAU DE TORTUE',         1),
  ('BOUCHON PPR 40',            1),
  ('BOUCHON PPR 32',            2),
  ('BOUCHON PPR 25',            3),
...
to_insert AS (
  ...
  FROM tmp_seed_plomberie_ppr_bouchons s
  WHERE NOT EXISTS (
...
SET
  ...
  updated_at   = NOW()
FROM public.stock_categories c, tmp_seed_plomberie_ppr_bouchons s
WHERE lower(trim(a.nom)) = lower(trim(s.nom))
  AND (
...
BEGIN
  IF EXISTS (
    ...
    WHERE (a.barcode_value IS NULL OR trim(a.barcode_value) = '')
      ...
      AND EXISTS (
        SELECT 1 FROM tmp_seed_plomberie_ppr_bouchons s
        WHERE lower(trim(a.nom)) = lower(trim(s.nom))
      );
  END IF;
END $bc$;
...
WHERE NOT EXISTS (
  ...
  WHERE m.article_id = a.id
    AND (
      coalesce(m.payload->>'source', '') = 'seed_plomberie_ppr_bouchons'
      OR (
...
FROM public.stock_articles a
JOIN tmp_seed_plomberie_ppr_bouchons s ON lower(trim(a.nom)) = lower(trim(s.nom))
LEFT JOIN public.stock_levels l
  ON l.article_id = a.id
 AND lower(trim(coalesce(l.emplacement, ''))) = 'depot lakhyayta'
ORDER BY a.nom;

SELECT COUNT(*)::int AS articles_seed_ppr_bouchons
FROM public.stock_articles a
JOIN tmp_seed_plomberie_ppr_bouchons s ON lower(trim(a.nom)) = lower(trim(s.nom));
