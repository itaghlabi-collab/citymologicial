-- =============================================================================
-- CITYMO — Seed PLOMBERIE Consommable — PPR / divers (liste inventaire 16/09/2026)
-- Supabase → SQL Editor → coller tout → Run
-- Idempotent : n’ajoute pas de doublon (même nom), n’écrase pas une qté déjà présente.
-- Type : Consommable — Catégorie : PLOMBERIE — Emplacement : DEPOT LAKHYAYTA
-- =============================================================================
...
DROP TABLE IF EXISTS tmp_seed_plomberie_ppr_divers;
CREATE TEMP TABLE tmp_seed_plomberie_ppr_divers (
  nom text PRIMARY KEY,
  qty numeric NOT NULL
);

INSERT INTO tmp_seed_plomberie_ppr_divers (nom, qty) VALUES
  -- PPR (captures 1–2)
  ('COUDE PPR 32 x 32 x 90°',                 12),
  ('RÉDUCTION PPR 32 x 20',                    22),
  ('TÉ PPR 20 x 20 x 20',                      32),
  ('COLLIER PPR 32',                           15),
  ('COLLIER PPR 25',                           12),
  ('COLLIER PPR 20',                           20),
  ('RACCORD FEMELLE PPR 20 x 1/2"',             2),
  ('COUDE PPR 20 x 20 x 45°',                  33),
  ('TÉ FEMELLE PPR 25 x 1/2"',                  6),
  ('MANCHON PPR 20',                            4),
  ('RACCORD MÂLE PPR 20 x 1/2"',               31),
  ('RACCORD MÂLE PPR 25 x 3/4"',                2),
  ('COUDE PPR 32 x 32 x 45°',                   3),
  ('COUDE FEMELLE PPR 25 x 3/4"',               2),
  ('MANCHON PPR 25',                           25),
  ('COUDE APPLIQUE PPR 20 x 1/2"',              1),
  ('TÉ PPR 20 x 20 x 1/2"',                     1),
  ('ROBINET D''ARRÊT PPR 25 x 25 x 3/4"',       1),
  ('COUDE PPR 25 x 25 x 90°',                   2),
  ('COUDE PPR 25 x 25 x 45°',                   2),
  -- PPR + divers (capture 3)
  ('RACCORD UNION 1" x 1/4"',                   1),
  ('MANCHON PPR 20 x 20',                       2),
  ('MANCHON PPR 32 x 32',                       4),
  ('TÉ FEMELLE PPR 20 x 20 x 1/2"',            12),
  ('TÉ PPR 25 x 25 x 25',                       6),
  ('COUDE FEMELLE PPR 20 x 1/2"',              20),
  ('COUDE PPR 20 x 20 x 90°',                  31),
  ('BOUCHON GALVANISÉ 3/4"',                    5),
  ('BOUCHON EN CUIVRE 3/4"',                    1),
  ('BOUCHON GALVANISÉ 1/2"',                    1),
  -- Cuivre / acier / robinets (capture 4)
  ('COUDE EN CUIVRE 3/4"',                      1),
  ('COUDE 16 x 1/2"',                           1),
  ('RÉDUCTION EN CUIVRE 1/2" x 3/4"',            1),
  ('MAMELON MÂLE/FEMELLE 1/2" x 1/2"',          1),
  ('TÉ GALVANISÉ 1" x 1/4"',                    5),
  ('COUDE EN ACIER 1"',                         6),
  ('COUDE EN ACIER 1" 1/4',                     6),
  ('ROBINET D''ÉQUERRE 1/2" x 3/8"',            3),
  ('ROBINET D''ÉQUERRE 1/2" x 3/4"',            3),
  ('ROBINET D''ÉQUERRE 1/2" x 1/2"',            2),
  -- Galva / PVC / vanne (capture 5)
  ('BOUCHON PVC 63',                            2),
  ('RACCORD MÂLE GALVANISÉ 1" x 1/4"',          6),
  ('MANCHON GALVANISÉ 1"',                      1),
  ('MANCHON GALVANISÉ 1" x 1/4"',               2),
  ('COUDE GALVANISÉ 1"',                        3),
  ('BOUCHON GALVANISÉ 1" x 1/4"',               1),
  ('VANNE 1"',                                  1);
...
to_insert AS (
  ...
  FROM tmp_seed_plomberie_ppr_divers s
  WHERE NOT EXISTS (
...
SET
  ...
  updated_at   = NOW()
FROM public.stock_categories c, tmp_seed_plomberie_ppr_divers s
WHERE lower(trim(a.nom)) = lower(trim(s.nom))
  AND (
...
BEGIN
  IF EXISTS (
    ...
    WHERE (a.barcode_value IS NULL OR trim(a.barcode_value) = '')
      ...
      AND EXISTS (
        SELECT 1 FROM tmp_seed_plomberie_ppr_divers s
        WHERE lower(trim(a.nom)) = lower(trim(s.nom))
      );
  END IF;
END $bc$;
...
WHERE NOT EXISTS (
  ...
  WHERE m.article_id = a.id
    AND (
      coalesce(m.payload->>'source', '') = 'seed_plomberie_ppr_divers'
      OR (
...
FROM public.stock_articles a
JOIN tmp_seed_plomberie_ppr_divers s ON lower(trim(a.nom)) = lower(trim(s.nom))
LEFT JOIN public.stock_levels l
  ON l.article_id = a.id
 AND lower(trim(coalesce(l.emplacement, ''))) = 'depot lakhyayta'
ORDER BY a.nom;

SELECT COUNT(*)::int AS articles_seed_ppr_divers
FROM public.stock_articles a
JOIN tmp_seed_plomberie_ppr_divers s ON lower(trim(a.nom)) = lower(trim(s.nom));
