-- =============================================================================
-- CITYMO — Seed PLOMBERIE Consommable — Sanitaire / accessoires (16/09/2026)
...
DROP TABLE IF EXISTS tmp_seed_plomberie_sanitaire;
CREATE TEMP TABLE tmp_seed_plomberie_sanitaire (
  nom text PRIMARY KEY,
  qty numeric NOT NULL
);

INSERT INTO tmp_seed_plomberie_sanitaire (nom, qty) VALUES
  ('SÈCHE-MAINS STIEBEL ELTRON',              3),
  ('CLAPET ANTI-RETOUR LUXWARES',             1),
  ('ABATTANT WC CREAVIT',                     4),
  ('ABATTANT WC NKP SLIM',                    2),
  ('PORTE-GOBELET',                           7),
  ('CANIVEAU EN BRONZE 10 x 50 CM',           1),
  ('CANIVEAU EN INOX 40 CM',                  1),
  ('ROBINET D''ARRÊT CREAVIT',                2),
  ('BONDE POUSSOIR CREAVIT',                  1),
  ('BONDE POUSSOIR SANICA',                   4),
  ('BONDE POUSSOIR CLEMER',                   1),
  ('PLAQUE POUSSOIR CHROMÉE GROHE',           2),
  ('BONDE POUSSOIR TIS ROSIPANIA',            2),
  ('SIPHON EN INOX VIEGA',                    4),
  ('ROBINET D''ARRÊT NOIR CREAVIT',           1);
...
to_insert AS (
  ...
  FROM tmp_seed_plomberie_sanitaire s
  WHERE NOT EXISTS (
...
SET
  ...
  updated_at   = NOW()
FROM public.stock_categories c, tmp_seed_plomberie_sanitaire s
WHERE lower(trim(a.nom)) = lower(trim(s.nom))
  AND (
...
BEGIN
  IF EXISTS (
    ...
    WHERE (a.barcode_value IS NULL OR trim(a.barcode_value) = '')
      ...
      AND EXISTS (
        SELECT 1 FROM tmp_seed_plomberie_sanitaire s
        WHERE lower(trim(a.nom)) = lower(trim(s.nom))
      );
  END IF;
END $bc$;
...
WHERE NOT EXISTS (
  ...
  WHERE m.article_id = a.id
    AND (
      coalesce(m.payload->>'source', '') = 'seed_plomberie_sanitaire'
      OR (
...
FROM public.stock_articles a
JOIN tmp_seed_plomberie_sanitaire s ON lower(trim(a.nom)) = lower(trim(s.nom))
LEFT JOIN public.stock_levels l
  ON l.article_id = a.id
 AND lower(trim(coalesce(l.emplacement, ''))) = 'depot lakhyayta'
ORDER BY a.nom;

SELECT COUNT(*)::int AS articles_seed_sanitaire
FROM public.stock_articles a
JOIN tmp_seed_plomberie_sanitaire s ON lower(trim(a.nom)) = lower(trim(s.nom));
