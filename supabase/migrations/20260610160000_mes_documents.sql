-- CITYMO — GED Module 1 : Mes documents (document_folders + documents)
...
CREATE INDEX IF NOT EXISTS idx_document_folders_department
  ON public.document_folders (department) WHERE is_deleted = FALSE;
...
ALTER TABLE public.document_folders ADD COLUMN IF NOT EXISTS parent_id UUID REFERENCES public.document_folders(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_documents_uploaded_by
  ON public.documents (uploaded_by) WHERE is_deleted = FALSE;

CREATE INDEX IF NOT EXISTS idx_documents_department
  ON public.documents (department) WHERE is_deleted = FALSE;
