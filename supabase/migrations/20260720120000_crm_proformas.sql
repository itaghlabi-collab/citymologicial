-- =============================================================================
-- CRM Factures Proforma — tables dédiées + lien inverse facture → proforma
-- Numérotation PF-YYYY-##### indépendante de FAC / AC / PR
--
-- Ne modifie PAS : generateCrmFactureNumero, type facture/acompte, paiements,
-- sync caisse, ni la séquence FAC.
--
-- ROLLBACK (manuel) :
--   ALTER TABLE public.crm_factures DROP CONSTRAINT IF EXISTS crm_factures_proforma_id_fkey;
--   ALTER TABLE public.crm_factures DROP COLUMN IF EXISTS proforma_id;
--   DROP TABLE IF EXISTS public.crm_proforma_lignes CASCADE;
--   DROP TABLE IF EXISTS public.crm_proformas CASCADE;
-- =============================================================================
...
-- 2) Lignes proforma (miroir crm_facture_lignes / crm_devis_lignes)
...
-- 3) Lien inverse facture → proforma d'origine
--    Affichage : « Issue de la Proforma PF-… »
...
-- 4) RLS — même modèle ouvert que crm_factures (durcissement erp_can possible ensuite)
